...
def prepare_ligand(mol,ligand_id:str,target_id:str,out_dir:str,align_pivot:Iterable,hydrate:bool=False,save_sdf:bool=True,preparation_config:dict=None,*args,**kwargs):
    mol = Chem.Mol(mol)
    ...
    if save_sdf:
        return {"ligand_id":f"{ligand_id}","target_id":target_id,"conf_pdbqt":tuple(confs),"conf_sdf":tuple(sdf),"box_center":align_pivot,"box_size":opt_size,"n_conf":n_conf}
    else:
        return {"ligand_id":f"{ligand_id}","target_id":target_id,"conf_pdbqt":tuple(confs),"box_center":align_pivot,"box_size":opt_size,"n_conf":n_conf}
...
def prepare_target(file:str,
    ...
    *args,**kwargs):
    ...
    with open(path.join(outdir,f"{target_id}_box.pdb"),"w") as f:
        f.write(box_to_pdb_string(box["box_center"],box["box_size"]))
...
class MeekoPipeline(PipelineTemplate):
    ...
    def optimize_ligand(self,opt_f:Callable=None,n_conf:int=1,protonate:bool=True,pH_min:float=None,pH_max:float=None,*opt_args,**opt_kwargs):
        ...
        if opt_f:
            ...
            optimized_sets = ligands.to_dict("records")
        ...
        if protonate:
            ...
            scrub=Scrub(ph_low=pH_min,ph_high=pH_max,skip_gen3d=True)
            ...
            if n_conf ==1:
                ...
                for ligand in tqdm(optimized_sets,desc=f"Protonation pH range:{pH_min}-{pH_max}",position=0):
                    try:
                        ...
                        print(f"Ligand {ligand["id"]} is not protonable correctly then skipped")
                        continue
        ...
        **kwargs
        ...
        if not box:
            ...
            for row in targets.itertuples(index=False):
                box[row.target_id] = row.ligand_box if row.ligand_box else None
        
        tasks = (
            (row.file, row.target_id, row.altloc, box.get(row.target_id))
            for row in targets.itertuples(index=False) if box.get(row.target_id) 
        )
    ...
    def init_docking(self, safety_factor: float = 0.9, **docking_kwargs):
        ...
        if self.ligands.shape[0] > usable_cores:
            self.__setattr__("docking_n_work",usable_cores)
            docking_kwargs["cpu"] = 1
        else:
    ...
    def screen_single_target(self,dock_f:Callable,target_id:str,report_fn:str=None,**dock_kwargs):
        ...
        with tqdm(total=self.replicates,desc=f"Docking {target_id}, replicates {self.replicates}",position=0) as replicate_bar:
            ...
            for replicate in range(1, self.replicates + 1):
                ...
                total_tasks = self.ligands["n_conf"].sum()
    ...
    def analyze_interactions(self,target_id:str):
        ...
        analysis = analysis.drop_duplicates(subset=["ligand_id", "target_id", "file"])
    ...
    def create_report(self):
        ...
        output_file = path.join(
            self.result_workpath,
            f"{self.exp_id}_{self.run_name}.xlsx"
        )

        report = self.__getattr__("report")

        report.drop(columns=["file"], inplace=True, errors="ignore")
        ...
        with pd.ExcelWriter(output_file, engine="xlsxwriter") as writer:
            ...
            for target_id, target_df in report.groupby("target_id"):
                ...
                # Normalize interaction columns that are present
                ...
                main_df = target_df.reindex(
                    columns=main_cols
                ).copy()

                # Interaction sheet
                # Missing interaction columns are automatically filled with NaN
                ...
                interaction_df = target_df.reindex(
                    columns=interaction_cols_full
                ).copy()
