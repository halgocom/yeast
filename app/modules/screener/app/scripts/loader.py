"""
From https://github.com/DrrDom/rdkit-scripts/blob/master/read_input.py
Original Author Github: https://github.com/DrrDom 
...
def __read_sdf(fname, input_format, id_field_name=None, sanitize=True):
    if input_format == 'sdf':
        ...
        suppl = Chem.ForwardSDMolSupplier(gzip.open(fname,"rb"), sanitize=sanitize, removeHs=False)
    else:
        return
    for mol in suppl:
        if mol is not None:
            ...
            yield PropertyMol(mol), mol_title,Chem.MolToSmiles(mol)


def __read_sdf_confs(fname, input_format, id_field_name=None, sanitize=True, sdf_confs=False):
    title = None
    for mol, mol_title,smiles in __read_sdf(fname, input_format, id_field_name, sanitize):
        if sdf_confs:
            if title is None:
                m = mol
                ...
                yield m, title,Chem.MolToSmiles(m)
                m = mol
                conf = assign_mol_props_to_conf(m, m.GetConformer(0))
                title = mol_title
        else:
            yield mol, mol_title,Chem.MolToSmiles(mol)
    if sdf_confs:
        yield m, title, Chem.MolToSmiles(m)
...
def __read_csv(csv_path:str,sanitize:bool=True,sep:str='\n'):
    with open(csv_path, newline=sep) as file:
        reader = csv.DictReader(file)
        for row in reader:
            ligand_name=row['id'].replace("-", "_").replace("/", "")

            yield Chem.MolFromSmiles(row["smiles"],sanitize=sanitize),ligand_name,row["smiles"]
...
def read_input(fname, input_format=None, id_field_name=None, sanitize=True, sdf_confs=False, sep='\t',newl='\n'):
    """
    ...
    if fname is None:    # handle STDIN
        ...
        suppl = __read_csv(os.path.abspath(fname), sanitize, sep=newl)
    else:
        raise Exception("Input file format '%s' is not supported. It can be only sdf, sdf.gz, smi, pkl." % input_format)
    for mol, mol_name,smiles in suppl:
        yield mol, mol_name, smiles
